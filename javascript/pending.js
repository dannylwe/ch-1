    ...
    .then(res=> {
        ...
        res.data['item info'].forEach(post => {
            if(post.status==='cancelled'){
                ...
                <td><i class="far fa-times-circle" id="cancel-sign" style="display:none;"></i></td>    
                </tr>`
            }else{
            output += `<tr>
            <td style = "width: 10%">${post.parcel_id}</td>
            ...
            <td style = "width: 15%">${post.nickname}</td>
            <td style = "width: 10%">${post.status}</td>
            <td><i class="far fa-times-circle" id="cancel-sign" onclick=cancelParcel(${post.parcel_id});></i></td>
            </tr>`
            }
        });
        output += '</div>';
